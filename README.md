...
**交付状态：0.1.0 已由用户在 GitHub 构建成功。0.2.0 的源码和配置检查已完成，但新版尚未编译和真机验证。**

## 0.2.0 更新

- 图标大小：32–80 点，默认 48 点。
- 图标不透明度：20%–100%，默认 100%；越低越透明，图标和角标一起调整。
- 平时圆形图标中心停在屏幕右边缘，露出一半。
- 新通知让对应 App 图标完整伸出，3 秒后缩回；连续通知从最后一次重新计时。
- 新图标滑出并淡入；删除或清空气泡时滑走并淡出；伸出、缩回和大小变化均有过渡动画。
- 遵循系统减弱动态效果设置。

### 更新已有 GitHub 仓库

解压新版，打开 `NotifyBubbles` 文件夹，把里面的 `Sources`、`Preferences`、`Tests`、`scripts`、`control` 和 `README.md` 拖入仓库 **Add file → Upload files** 页面，提交覆盖即可。本次工作流未变，不必重建 `.github/workflows/build.yml`。提交后会自动编译，下载新的 `NotifyBubbles-RootHide`。安装其中版本为 0.2.0 的 `.deb`，随后重启桌面。
...
- [ ] 新通知滑出淡入，3 秒后缩回；第 2 秒又收到通知时，从新通知重新计时。
- [ ] 两个 App 的缩回计时互不干扰；缩回中收到消息能平滑再次伸出。
- [ ] 点击清空时滑走淡出；清空后立即收到新消息不会被旧动画误隐藏。
- [ ] 无新通知时不显示任何气泡。
