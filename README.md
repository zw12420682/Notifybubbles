...
另外将 `.github/workflows/build.yml` 替换为新版。提交后在 Actions 查看最新构建，成功后下载 Artifacts 中 NotifyBubbles-RootHide，解压安装 0.16.0 的 deb 并重启桌面。
...
- 点击当前浮窗图标且没有待处理通知：**返回 App 内上一页**（等同从屏幕左边缘向右滑的返回手势）。
- 没有待处理通知且目标不是当前浮窗：通过 TrollOpen 打开浮窗；目标为全屏当前 App 时使用其专用前台转分屏入口。
- 点击边缘图标时，伸出动画与点击处理同时开始。
- 长按当前分屏浮窗对应的图标约 0.6 秒：**切换分屏窗口方向（竖屏 ↔ 横屏）**，等同绿色交互条长按旋转。其余图标长按仍关闭图标，保留破裂动画。
- 双击当前分屏浮窗对应的图标：**关闭当前分屏窗口**。仅当前浮窗对应的图标响应，双击其它图标无操作。
...
2. 浮窗中无通知时点击气泡返回 App 内上一页（边缘右滑返回）；有通知时优先跳转通知。
3. 分屏状态下长按气泡全屏当前分屏窗口；非分屏图标长按仍破裂关闭。
