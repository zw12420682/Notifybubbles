...
**交付状态：0.1.0 已由用户在 GitHub 构建成功。0.3.0 的源码和配置检查已完成，但新版尚未编译和真机验证。**
...
- 点击使用本 App 最近仍有效的系统通知默认动作，保留所有图标，不操作其他 App 的通知或角标。
- 无有效通知的保留图标点击时不跳转，长按仍可关闭。
...
解压新版，把 `NotifyBubbles` 里的 `Sources`、`Preferences`、`Tests`、`scripts`、`control` 和 `README.md` 上传到仓库根目录覆盖。工作流不用修改。提交后自动构建，下载新构建的 `.deb`（版本 0.3.0）。
...
| 点击 | 执行该 App 最新有效通知的系统默认动作；所有气泡保持，其他 App 不受影响 |
...
- [ ] 通知中心清除消息、App 撤回通知后，相关图标仍保留，但不再打开已经失效的通知。
