...
另外将 `.github/workflows/build.yml` 替换为新版。提交后在 Actions 查看最新构建，成功后下载 Artifacts 中 NotifyBubbles-RootHide，解压安装 0.20.0 的 deb 并重启桌面。
...
自 0.19.0 起（手势最终形态）：单击当前分屏浮窗的气泡 → **全屏**；长按该气泡 → **无动作**；长按其余气泡 → 破裂消失且该 App **退出后台**。
